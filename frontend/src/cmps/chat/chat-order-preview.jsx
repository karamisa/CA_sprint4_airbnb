...
export function ChatOrderPreview({
  order,
  onSetCurrOrder,
  currOrder,
  loggedInUser,
  ...
  const className =
    currOrder && currOrder._id === order._id
      ...
      : 'chat-order-preview'
  const contact =
    order.hostId === loggedInUser._id ? order.buyer : order.stay.host
      ...
      <div
        ...
        <div className='chat-order-details flex'>
          <div className='chat-order-status flex justify-between'>
            {/* <span className={`chat-show-status ${order.status}`}>
              {order.status}
            </span> */}
            {/* {order.msgs[order.msgs.length - 1]?.createdAt && (
              <span className='chat-order-time'>
                {hours}:{minutes}
              </span>
            )} */}
          </div>
          ...
          <div className='chat-mini-details'>
            <span className={`chat-show-status`}>
              {/* <span className={`chat-show-status ${order.status}`}> */}{' '}
              {order.status}
            </span>
            {' \u2022 '}
            <span className='chat-mini-booking'>
              {checkInDate}-{checkOutnDate}
            </span>

            {/* <span className="chat-mini-type">{order.stay.type}</span> */}
          </div>
        </div>
      </div>
